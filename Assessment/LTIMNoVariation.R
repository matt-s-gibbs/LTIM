...
DifferencePlot<-function(X1,X2,Name,xlab,binwidth)
{
  ...
  ggsave(paste0("Assessment/OutputNoVariation/",Name,".png"),p,width=16,height=22,units="cm",dpi=300)
...
Runs<-c("-Historic.txt","-NoVariation.txt")
RunNames<-c("Observed","No Variation")
...
pWL2<-ggplot(A)+geom_line(aes(x=Index,y=Value,colour=Scenario))+
  ...
  ggsave(paste0("Assessment/OutputNoVariation/WaterLevelUpper_Subset.png"),pWL2,width=16,height=10,units="cm",dpi=300)
ggsave(paste0("Assessment/OutputNoVariation/Velocity_Subset.png"),pv,width=16,height=10,units="cm",dpi=300)
